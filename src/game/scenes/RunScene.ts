...
const BOSS_CLOSE_HIT = 0.4; // 피격 시 근접도 증가(3연속 실수 → 1.0이면 붙잡힘)
...
// 보스전 전용 풀 — 회복템만. 무적/철벽/비행 등이 보스 위협(공격·그랩)을 무력화하지
// 않도록 차단(투사체를 끄는 것과 동일 취지). 큰 회복은 드물게 두어 역전 여지만 남김.
const BOSS_ITEM_POOL: ItemKind[] = (["americano", "vacation", "holiday"] as ItemKind[]).flatMap(
  (k) => Array<ItemKind>(ITEM_WEIGHTS[k]).fill(k),
);
...
export class RunScene extends Phaser.Scene {
  ...
  private updateBoss(dt: number) {
    ...
    // 근접도 1.0 → 붙잡힘. 반드시 회복(decay)보다 먼저 검사 — 피격으로 1.0(상한)에 닿은
    // 값을 decay가 한 프레임 먼저 깎으면 영원히 <1이 되어 그랩이 발동하지 않는다(버그 수정).
    if (this.bossClose >= 1) {
      this.grabPlayer();
      return;
    }
  ...
  private spawnItem() {
    const pool = this.bossActive ? BOSS_ITEM_POOL : ITEM_POOL;
